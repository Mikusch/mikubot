                                ...
                                if (messageIdOption == null || messageIdOption.isBlank()) {
                                    ...
                                    channel.retrieveMessageById(messageId)
                                            ...
                                            .flatMap(message -> message == null
                                                    ...
                                                    : hook.editOriginal(
                                                            "Thought of a specific message: " + message.getJumpUrl()))
                                            .queue();
                                }
                            },
                            error -> hook.editOriginal("Failed to retrieve webhook: %s".formatted(error.getMessage()))
                                ...
                                CompletableFuture.runAsync(() -> {
                                    ...
                                    if (!messagesToSave.isEmpty()) {
                                        List<Long> messageIds = messagesToSave.stream()
                                                .map(ThinkerMessage::getMessageId)
                                                .toList();
                                        Set<Long> existingIds = new HashSet<>(messageRepository.findExistingMessageIds(
                                                config.getWebhookId(), messageIds));
            ...
            try {
                ...
                if (presetMessageId != null) {
                    ...
                    return channel.retrieveMessageById(presetMessageId)
                            .flatMap(msg -> sendMessageImmediate(msg, config))
                            .onSuccess(message -> log.info("Successfully sent preset message: {}", message));
                }
        ...
        configs.values().forEach(config -> {
            jda.retrieveWebhookById(config.getWebhookId())
                    .flatMap(webhook -> {
                        ...
                        return channel.retrieveMessageById(channel.getLatestMessageId())
                                .flatMap(latestMessage -> retrieveAvgDurationBetweenMessagesInChannel(channel)
                                        .flatMap(duration -> {
                                            OffsetDateTime lastPosted = lastPostedTimes.getOrDefault(
                                                    config.getWebhookId(), OffsetDateTime.now());
                                            if (duration != null
                                                    && OffsetDateTime.now()
                                                            .isAfter(lastPosted.plus(duration.multipliedBy(
                                                                    config.getDurationMultiplier())))
                                                    && messageRepository.countByWebhookId(config.getWebhookId()) > 0) {
                                                if (latestMessage.getAuthor().getIdLong()
                                                        == webhook.getDefaultUser()
                                                                .getIdLong()) {
                                                    log.info(
                                                            "Latest message is already authored by webhook {}, refusing to post",
                                                            config.getWebhookId());
                                                    lastPostedTimes.put(
                                                            config.getWebhookId(), latestMessage.getTimeCreated());
                                                } else {
                                                    return sendRandomMessageImmediate(channel, config);
                                                }
                                            }
                                            log.debug(
                                                    "Not enough time has passed yet for webhook {}, refusing to post",
                                                    config.getWebhookId());
                                            return new CompletedRestAction<>(jda, null);
                                        }));
                    })
